...
def backward_hook(module, grad_input, grad_output):
    ...
    if isinstance(module, nn.LayerNorm):
        timers('layernorm_grads_all_reduce').start()
        #torch.distributed.all_reduce(grad_output[0])
        torch.cuda.synchronize()
        timers('layernorm_grads_all_reduce').stop()
        performance_data["execution_time"]["layernorm_grads_all_reduce_time_ms"] += timers('layernorm_grads_all_reduce').elapsed() * 1000

    # 监测 Embedding 梯度同步时间
    elif isinstance(module, LanguageModelEmbedding):
...
def backward_pre_hook_cpl(module, grad_output):
    torch.cuda.synchronize()
    torch.cuda.reset_peak_memory_stats()
    initial_mem = torch.cuda.memory_allocated() / (1024**2)

    timers('backward_cpl').start()  # 正确启动计时器

    if torch.distributed.get_rank() == 0:
        cpl.append(('backward_pre', 0, initial_mem))
        layer_name = module.__class__.__name__
        print(f"backward_pre: Layer: {layer_name}")

    return grad_output

def backward_hook_cpl(module, grad_input, grad_output):
    torch.cuda.synchronize()

    timers('backward_cpl').stop()  # 停止反向传播计时
    end_time = timers('backward_cpl').elapsed()
    ...
    if torch.distributed.get_rank() == 0:
        cpl.append(('backward', end_time, mem))
        print(f"backward: {end_time :.6f}")
...
def register_hooks(model):
    ...
    for name, module in model.named_modules():
        if isinstance(module, LanguageModelEmbedding) or isinstance(module, TransformerLayer):
        ...
        elif isinstance(module, ColumnParallelLinear) and name != "output_layer" and max_estimate == True:
            ...
            backward_pre_hook_fn = module.register_full_backward_pre_hook(backward_pre_hook_cpl)
            backward_hook_fn = module.register_full_backward_hook(backward_hook_cpl)
            hooks.append(forward_pre_hook_fn)
            hooks.append(forward_hook_fn)
            hooks.append(backward_pre_hook_fn)
            hooks.append(backward_hook_fn)
            max_estimate = False
        elif isinstance(module, ColumnParallelLinear) and name == "output_layer":
            forward_pre_hook_fn = module.register_forward_pre_hook(forward_pre_hook_cpl)
            forward_hook_fn = module.register_forward_hook(forward_hook_cpl)
            hooks.append(forward_pre_hook_fn)
            hooks.append(forward_hook_fn)
...
if __name__ == "__main__":
    ...
    for i in range(0,5,1):
        if i == 3 and torch.distributed.get_rank() == 0:
            ...
            print("iter_begin: "+str(iter_begin_time))
        optim.zero_grad()
        ...
        if i == 3 and torch.distributed.get_rank() == 0:
            ...
            print("iter_end: "+str(iter_end_time))
...
#######################################################################################
    if torch.distributed.get_rank() == 0: 
        ...
        layer_compute_time.append((cpl[3][1]-cpl[2][1] + (cpl[5][1]-cpl[4][1])/(cpl[1][1]-cpl[0][1])*(cpl[3][1]-cpl[2][1])) * 1000)
        layer_memory.append(cpl[3][2]-cpl[2][2] + (cpl[5][2]-cpl[4][2])/(cpl[1][2]-cpl[0][2])*(cpl[3][2]-cpl[2][2]))
        ...
        output_file = os.path.join(output_dir, f"DeviceType.A100_tp{2}_bs{8}.json")

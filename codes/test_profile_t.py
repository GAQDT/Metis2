...
model = t_p.tensor_parallel(model, device_ids=devices)
...
def forward_pre_hook(module, input):
    ...
    if device != torch.device("cuda:0"):  # 只在cuda:0上触发
        return input
    ...
    print("fpre: "+str(start_time-begin_time))
    return input
...
def forward_hook(module, input, output):
    ...
    print("f: "+str(end_time-begin_time))
    return output
...
def backward_pre_hook(module, grad_output):
    ...
    print("bpre: "+str(start_time-begin_time))
    return grad_output
...
def backward_hook(module, grad_input, grad_output):
    ...
    print("b: "+str(end_time-begin_time))
    return grad_input
...
# 如果使用了 CUDA，还需要设置 CUDA 随机种子
if torch.cuda.is_available():
    torch.cuda.manual_seed_all(42)
# 模拟一个输入，进行性能测量
model_device = next(model.parameters()).device
dummy_input = torch.randint(0, 50257, (bs, 1024), device=model_device).long()
labels = dummy_input.clone().to(model_device)
